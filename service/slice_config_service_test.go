...
func SliceConfigReconciliationCompleteHappyCase(t *testing.T) {
	workerSliceGatewayMock, workerSliceConfigMock, _, workerServiceImportMock, _, clientMock, sliceConfig, ctx, sliceConfigService, requestObj, mMock, ipamMock := setupSliceConfigTest("slice_config", "namespace")
...
func SliceConfigReconciliationNoNetCompleteHappyCase(t *testing.T) {
	_, workerSliceConfigMock, _, _, _, clientMock, sliceConfig, ctx, sliceConfigService, requestObj, mMock, ipamMock := setupSliceConfigTest("slice_config", "namespace")
	mMock.On("WithProject", mock.AnythingOfType("string")).Return(&metrics.MetricRecorder{}).Once()
	ipamMock.On("InitializePool", sliceConfig.Name, sliceConfig.Spec.SliceSubnet).Return(nil).Once()
...
func setupSliceConfigTest(name string, namespace string) (*mocks.IWorkerSliceGatewayService, *mocks.IWorkerSliceConfigService, *mocks.IServiceExportConfigService, *mocks.IWorkerServiceImportService, *mocks.IWorkerSliceGatewayRecyclerService, *utilMock.Client, *controllerv1alpha1.SliceConfig, context.Context, SliceConfigService, ctrl.Request, *metricMock.IMetricRecorder, *mocks.IPAMAllocator) {
	...
	return workerSliceGatewayMock, workerSliceConfigMock, serviceExportConfigMock, workerServiceImportMock, workerSliceGatewayRecyclerMock, clientMock, sliceConfig, ctx, sliceConfigService, requestObj, mMock, ipamMock
}
